...
> **Cập nhật:** 2026-06-09. **Tiến độ:** **P0–P7 ĐÃ MERGE vào `main`** (P7 đã push origin, CI+CD GitHub Actions XANH). **P8a (Measurement foundation, C-đánh giá) ĐÃ XONG trên branch `phase8a`** — 12 commit, 11 task subagent-driven (implementer → spec review → quality review → hardening → final opus review = **READY TO MERGE, Critical=0**). **152 test xanh, ruff sạch** (133 + 19 mới). P8a gồm: `src/experiments/` = harness N-run subprocess (`runner.py`) + 6 metric collector (`metrics/{model_perf,business,resource,drift_quality,operational}` + maturity) + stats (Wilcoxon + bootstrap CI) + **ML Test Score assessor** (`maturity.py` evidence auto-verify + MIN 4 mục + Google/Azure) + `mltest_manifest.yaml` + CLI (`python -m src.experiments assess`); **provenance §9.6 ĐÃ XỬ LÝ** (`run_training` log dataset lineage params). P8a = **hạ tầng đo, unit-test trên synthetic, KHÔNG sinh số liệu kết quả**. **▶ HÀNH ĐỘNG NGAY (session mới): (1) merge `phase8a` → main (HV chọn); (2) execute P8b** — Exp-1..6 runners + `raw_data` full run (~60min) + result tables, nạp dữ liệu thật vào hạ tầng P8a (plan: viết `docs/plans/<date>-phase8b-*.md`). Sau P8b: P9 (viết luận văn). Spec/plan P8a: `docs/superpowers/specs/2026-06-09-phase8a-foundation-design.md`, `docs/plans/2026-06-09-phase8a-foundation.md`.
...
- **P6 (Monitoring + auto-retrain, C8) ĐÃ MERGE:** drift covariate **PSI 3 tầng (tự code) + Evidently (chuẩn ngành, cross-check)** trên `predictions.jsonl`; **closed-loop drift→auto-retrain (sliding-window)→eval-gate→`ServingRuntime.reload()`** (RQ4/Exp-4); `/metrics` Prometheus scrape-ready. **Monitoring = năng lực C1** (KHÔNG abstraction; đối chứng = loop ON/OFF đo ở Exp-4). Tinh gọn chống over-engineering: server Prometheus/Grafana + dashboard → P7 (§9 mục 12). Plan: `docs/plans/2026-06-08-phase6-monitoring.md`. *(Lịch sử: P7 CI/CD + P8a Measurement foundation ĐÃ XONG sau P6 — xem header + §6 cho trạng thái hiện tại + việc tiếp theo.)*
- **152 test xanh, ruff sạch** (P0–P7 merged + P8a trên branch `phase8a`). Chạy `source .venv/bin/activate && pytest -q && ruff check .` (≈22s).
...
- **Git:** branch `main` (**P0–P7 merged**; chạy `git log -1` để xem HEAD), **KHÔNG có branch đang mở** (mỗi phase merge xong xóa branch). Remote `origin` = `git@github.com:ngocmx2104/MLOps-NWDAF-V2.git`. **ĐÃ push `origin/main`** (P0–P7) — `ci.yml`+`cd-pipeline.yml` chạy **XANH** lần đầu trên GitHub Actions (3.14+torch cài được trên runner, không cần fallback 3.12). HV vẫn tự quyết các push sau. **P8 trở đi: tạo branch `phaseN-*` từ main.** (Note: 1 commit doc-fix có thể đang local-ahead — sẽ ride với push code kế.)
...
| **P8a** | Measurement foundation | đánh giá | ✅ **xong branch `phase8a`** (chờ merge) — `src/experiments/` harness + 6 collector + Wilcoxon/CI + **ML Test Score assessor** (evidence auto-verify) + provenance §9.6; 152 test xanh; final review READY TO MERGE. Plan: `docs/plans/2026-06-09-phase8a-foundation.md` |
| **P8b** | Experiments + result tables | đánh giá | ⬜ chưa — Exp-1..6 runners + ML Test Score 28-test thật + Wilcoxon N≥10; **chạy full `raw_data` (~60min)**; nạp dữ liệu thật vào hạ tầng P8a |
...
6. ✅ **ĐÃ XỬ LÝ (P8a, commit `d1db8da`)** — Provenance/dataset-lineage: `run_training` (iforest path) giờ log `ctx` lineage dạng **params** (`dataset_id`/`feature_version`/`source_snapshot_id`/`dataset_layer`/`row_count`) qua `tracker.log_params` — KHÔNG log full parquet mỗi run (đúng chủ trương N≥10 tiết kiệm storage). Test `tests/training/test_provenance.py` khẳng định đủ 5 key. Củng cố delta traceability C1-vs-C0 (RQ3). **Còn nợ (→ P8b):** LSTM path chưa log lineage (gắn §9.7 data-path tách rời); `tracker.log_dataset`/`log_artifact` (vd IForest `fit_summary` JSON sidecar) chưa gọi.
...
9. ⏭️ **DEFER → P8 (quyết định Q4 brainstorm P7, tránh kéo torch-Docker vào P7)** — LSTM serving qua registry chưa self-contained: registry chỉ lưu meta bundle; `.pt` trỏ absolute path lúc train → `RegistryModelLoader` đã WARN, iforest (model chính) KHÔNG ảnh hưởng; P5 dùng PathLoader cho lstm cục bộ (single-node OK). **P7:** log cả `.pt` + resolve relative để lstm serving chạy remote/Docker.
10. ✅ **ĐÃ XỬ LÝ (P7 T12)** — docker-compose cần model có sẵn: `MLOPS_SERVING_MODEL_PATH=/app/models/model_iforest.joblib` mount từ `artifacts/models` (gitignored) → fresh clone phải train model trước (nếu không container start nhưng `/predict` lỗi runtime). Thêm comment/README hoặc healthcheck ở P7.
...
13. ✅ **ĐÃ XỬ LÝ (P7 T3, commit `e917838`)** — Eval-gate registry governance: `run_training` move alias MLflow `staging` → model retrain TRƯỚC eval-gate; khi gate REJECT, loop skip `reload()` (runtime in-memory giữ model cũ — đúng trong 1 Exp-4 run) NHƯNG alias `staging` persisted vẫn trỏ model bị từ chối → reload/restart sau load nhầm. Đã ghi comment limitation trong `retrain.py`. **P7:** tách register-khỏi-promote trong `run_training` (đăng ký alias `candidate`, gate mới promote `staging`) hoặc revert alias khi reject. (Trong Exp-4 single-run, gate in-memory vẫn đúng → không chặn P6.)
...
16. **[NỢ P8a→P8b] Minor polish hạ tầng experiments (final review opus, non-blocking):** (a) `runner._bind` chỉ substitute `{seed}` ở **argv**, KHÔNG ở `RunConfig.env` — spec §4 nói "arg/env"; P8b nếu cần seed-qua-env thì mở rộng hoặc ghi rõ seed đi qua argv; (b) `runner.run_n(rc, …)` thiếu type hint `rc: RunConfig` (cosmetic); (c) `experiments.schema.ExperimentConfig` trùng tên `tracking.schema.ExperimentConfig` (khác module, không shadow runtime — đổi tên nếu gây nhầm ở P8b); (d) `tracking_overhead` + `events_from_logs` là seam P8a→P8b (chưa có caller in-repo — đúng thiết kế, P8b sẽ gọi). **[NỢ P8a→P8b nội dung]:** điền `mltest_manifest.yaml` từ 5 seed-test → đủ 28 test Breck (mỗi điểm trỏ artifact thật verify được); thêm clearml pipeline-level smoke trước full sweep (§9 mục 8); xử §9.7 (LSTM data-path + lineage), §9.14 (Exp-4 harness window), §9.1 (`raw_data` lock).

---

## 10. BẮT TAY LÀM TIẾP — hành động cụ thể ngay

1. `cd "/Users/ngocmx/Thạc Sĩ/MLOps_Thesis_Ver2" && source .venv/bin/activate && git checkout phase8a && pytest -q && ruff check .` → xác nhận **152 xanh** (P8a trên branch `phase8a`, chưa merge). `git log --oneline main..HEAD` để xem 12 commit P8a. (Nếu `phase8a` đã merge + xóa thì `git checkout main`.)
2. **Merge P8a** (HV chọn): `git checkout main && git merge phase8a` (local fast-forward) → verify `pytest -q` → `git branch -d phase8a`. Push khi HV yêu cầu.
3. **Bắt đầu P8b (Experiments + result tables)** — theo flow §4: viết plan `docs/plans/<date>-phase8b-*.md` → branch `phase8b` từ main → subagent-driven. Nội dung: Exp-1..6 runners **nạp dữ liệu thật vào hạ tầng P8a** (`src/experiments/`) + `raw_data` full run (~60min, §9.1) + ML Test Score 28-test manifest thật + result tables + Wilcoxon N≥10. **Nợ kỹ thuật xử ở P8b:** §9 mục 7–8 (LSTM data-path + lineage), 14 (Exp-4 harness window), 16 (polish runner + điền manifest). Port harness/records/stats cũ `MLOps_Project/src/experiments/` (đã có hạ tầng P8a, chỉ nạp dữ liệu). (§9 mục 15 → P9.)
